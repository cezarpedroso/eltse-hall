...
import { Check, Clock3, UserMinus, Users } from 'lucide-react'
import { api, ApiError } from '../api'
import type { CurrentUser, ResidentAssistant, Schedule, Shift } from '../types'
...
export function ScheduleCalendar({ schedule, user }: { schedule: Schedule; user: CurrentUser }) {
...
function ShiftDetailsDialog({ schedule, shift, user, onClose }: { schedule: Schedule; shift: Shift | null; user: CurrentUser; onClose: () => void }) {
  const queryClient = useQueryClient()
  const toast = useToast()
  const isManager = user.role === 'HallDirector' || user.role === 'Admin'
  const [selectedUserId, setSelectedUserId] = useState('')
  const closeDialog = () => { setSelectedUserId(''); onClose() }
  const staff = useQuery({
    queryKey: ['admin-users'],
    queryFn: ({ signal }) => api<ResidentAssistant[]>('/api/admin/users', {}, signal),
    enabled: isManager && !!shift,
  })
  const refresh = async () => {
  ...
  const assignStaff = useMutation({
    mutationFn: (userId: string) => api(`/api/admin/shifts/${shift?.id}/assignments`, {
      method: 'POST', body: JSON.stringify({ userId, notes: null, overrideRules: true }),
    }),
    onSuccess: async () => { toast('Night shift assigned.'); await refresh() },
  })
  const removeStaff = useMutation({
    mutationFn: (assignmentId: string) => api(`/api/admin/shifts/${shift?.id}/assignments/${assignmentId}`, { method: 'DELETE' }),
    onSuccess: async () => { toast('Night shift unassigned.'); await refresh() },
  })
  ...
  return <Dialog open={!!shift} onClose={closeDialog} title={fullDate(shift.dutyDate)} className="shift-dialog">
    ...
    {isManager ? <section className="manager-assignment">
      <label htmlFor="shift-staff-member">Assign staff member</label>
      <div><select id="shift-staff-member" value={selectedUserId} onChange={(event) => setSelectedUserId(event.target.value)} disabled={staff.isLoading || assignStaff.isPending}>
